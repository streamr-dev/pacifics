 ...
 * @returns {Promise.<string>} created contract's address
 */
const createDeliveryContract = (parcelAddress, senderPostbox, receiverPostbox, receiver, minutes) => {
    ...
    return new Promise((done, fail) => {
        DeliveryContractCreator.createDeliveryContract(parcelAddress, senderPostbox, receiverPostbox, receiver, 0, 0, 0, minutes, (err, tx) => {
            if (err) { throw err }
            const filter = web3.eth.filter("latest")
            filter.watch(function (error, blockHash) {
                if (error) { throw error }
